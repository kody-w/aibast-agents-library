...
Shared Brainstem releases flow down from the upstream stable channel, but this repository is not a mirror. Preserve these AIBAST-owned surfaces during every sync:
...
Only repository-identity references are rewritten mechanically — the upstream repo identity becomes `microsoft/aibast-agents-library`, and Brainstem support drafts target `microsoft/aibast-agents-library`. The Tier-2 cloud implementation now lives in-repo under `rapp_cloud/` (see rapp/SUCCESSION.md for the full flow-down process). Installer mirrors under `docs/install.*` must remain byte-identical to their root counterparts.
