...
git clone --depth 1 --quiet https://github.com/microsoft/aibast-agents-library.git "$PROJECT_DIR"
...
if [ ! -f "$BIZ_HTML" ]; then
    ...
    curl -fsSL "https://raw.githubusercontent.com/microsoft/aibast-agents-library/main/business.html" -o "$BIZ_HTML" 2>/dev/null || true
fi
