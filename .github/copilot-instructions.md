...
Never overwrite AIBAST-owned content with upstream-stable equivalents: `agents/@aibast-agents-library/`, `registry.json`, `build_registry.py`, `rapp_cloud/`, root `README.md`/`index.html`/`CLAUDE.md`, `docs/index.html`, `docs/tutorial.html`, `docs/rapp-guide.html`, `.github/`, legal/governance files, `.vscode/`, and `tools/`. Rewrite only upstream repository-identity URLs to `microsoft/aibast-agents-library`. The Tier-2 cloud implementation lives in-repo at `rapp_cloud/`; `community_rapp/` is a legacy-compat path only.
