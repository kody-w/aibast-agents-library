...
#  One-liner: curl -fsSL https://raw.githubusercontent.com/microsoft/aibast-agents-library/main/rapp_cloud/install.sh | bash
# ═══════════════════════════════════════════════════════════════════════════════
set -e
...
# ── Constants ─────────────────────────────────────────────────────────────────
VERSION="1.0.0"
INSTALL_DIR="$HOME/.rappcloud"
REPO_DIR="$INSTALL_DIR/src"
SOURCE_DIR="$REPO_DIR/rapp_cloud"
...
install_rapp_cloud() {
    ...
    if [ -d "$REPO_DIR/.git" ]; then
        ...
        remote_version=$(git show origin/main:rapp_cloud/VERSION 2>/dev/null | tr -d '[:space:]' || echo "$local_version")
    ...
    if [ ! -f "$SOURCE_DIR/function_app.py" ]; then
        fail "Installation failed — RAPP Cloud source not found at $SOURCE_DIR"
    fi
    ...
    success "RAPP Cloud v${VERSION} ready at $SOURCE_DIR"
}
...
configure_openai() {
    ...
    case "$AUTH_CHOICE" in
        ...
        3|*)
            ...
            info "Edit ~/.rappcloud/src/rapp_cloud/local.settings.json later"
            api_key="<your-openai-api-key>"
            endpoint="https://<your-openai-resource>.openai.azure.com/"
            ;;
    esac
...
INSTALL_DIR="$HOME/.rappcloud"
SOURCE_DIR="$INSTALL_DIR/src/rapp_cloud"
VENV_DIR="$INSTALL_DIR/venv"

if [ ! -d "$SOURCE_DIR" ]; then
    echo "Error: RAPP Cloud not found at $SOURCE_DIR"
    echo "Run the installer: curl -fsSL https://raw.githubusercontent.com/microsoft/aibast-agents-library/main/rapp_cloud/install.sh | bash"
    exit 1
fi
...
print_success() {
    ...
    echo -e "${GREEN}${BOLD}"
    echo "  ═══════════════════════════════════════════════════"
    echo "  ✓ RAPP Cloud v${installed_version} installed!"
    echo "  ═══════════════════════════════════════════════════"
    echo -e "${NC}"
...
# Handle --help
if [[ "${1:-}" == "--help" ]] || [[ "${1:-}" == "-h" ]]; then
    echo "RAPP Cloud Installer"
    echo ""
    echo "Usage:"
    echo "  curl -fsSL https://raw.githubusercontent.com/microsoft/aibast-agents-library/main/rapp_cloud/install.sh | bash"
    echo "  ./install.sh"
    echo ""
