...
(function () {
  ...
  var TOOLS = [
    ...
    { type: "function", function: {
      name: "install_agent",
      description: "Install a RAPP agent .py into agents/ so it hot-loads. where:'twin' installs it onto the burrowed device — for on-device-only agents (subprocess to pac/az/gh, native libs, local files). Get the source with read_file or run_python (e.g. fetch from the AIBAST library) first.",
      parameters: { type: "object", properties: {
  ...
  function systemFor() {
    ...
    return SYSTEM_PROMPT + "\n\n=== BURROW IS ON — ONE NEIGHBORHOOD, TWO ADDRESSES ===\n" +
      ...
      "To run an on-device-only agent: (1) list_agents where:'twin' to see what the device already has, (2) if it's missing, get the source (read_file here, or run_python to fetch from the AIBAST library) and install_agent where:'twin', (3) run_agent where:'twin' by name — or just chat where:'twin' and let the twin pick the agent itself. " +
