...
| Upstream + LTS admin (deferred by owner decision, 2026-08-05) | Purge `tools/tool_assets/snapshot-1760370929454.html` from git history (`git filter-repo`/BFG + force-push). The blob is 30 MB, contains a real work identity and tenant identifiers, and sits in a commit that is an ancestor of **both** this fork and `microsoft/aibast-agents-library` — the two `SECURITY:` commits removed it from the tip only. Treat the identity as already disclosed. |
| ✅ Done (2026-08-05) | Workflow trigger signatures rotated by the owner. The code reads both from the environment; no signed URL ships in any agent template (gate T-SECRETS). |
| ✅ Done (2026-08-05), one human check outstanding | The shipped solution is now `MSFTAIBASMultiAgentCopilot_1_0_0_6.zip` with the appended `To` recipient, the hardcoded `Cc`, and the publisher email removed; `_1_0_0_5.zip` is retained as a versioned backup. Structurally verified (zip integrity, every workflow JSON and manifest still parses, 72/72 entries, `To` still bound to `triggerBody()['to']`) and gated by **T-SOLUTION**. **Outstanding:** import it once into Power Platform to confirm it still loads — that cannot be verified from here. |
