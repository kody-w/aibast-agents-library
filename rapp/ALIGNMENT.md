...
| **Upstream + LTS admin (BLOCKING, do before the repo is promoted)** | **Purge `tools/tool_assets/snapshot-1760370929454.html` from git history** (`git filter-repo`/BFG + force-push). The blob is 30 MB, contains a real work identity and tenant identifiers, and sits in a commit that is an ancestor of **both** this fork and `microsoft/aibast-agents-library` — the two `SECURITY:` commits removed it from the tip only. Treat the identity as already disclosed. |
| **Upstream (BLOCKING)** | **Rotate the two workflow trigger signatures** that were public in agent templates (Power Automate flow `074e7174…`, Logic App `e6a4ec20…`). The code now reads them from env, but a published signature is a published credential regardless. Independent probing suggests both flows are already deleted; rotate/confirm anyway. |
| **Upstream (BLOCKING)** | **Repack `MSFTAIBASMultiAgentCopilot_1_0_0_5.zip`**: `Workflows/RAPP-SendAnEmail-*.json` appends a real work address to `To` and an MCAP tenant UPN to `Cc` on every import, and `solution.xml` sets the publisher email to a personal address. Strip both, re-import once to confirm the solution still loads. |
