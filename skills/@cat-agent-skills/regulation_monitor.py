"""
Regulation Monitor — RAPP agent, mirrored from regulation_monitor.md
...
Do not edit this file. Edit regulation_monitor.md and re-run the exporter — the gate
re-derives this file and fails if the two have drifted apart.

Redistributed from CAT Agent Skills under MIT
with attribution. Original author: Jagmeet Chabra.
Origin: https://microsoft.github.io/cat-agent-skills/#regulation-monitor
"""
...
__manifest__ = {
    ...
    "description": "Returns the verified RAPP procedure for: Configure once, then on a schedule sweeps a locked list of authoritative sources (auto-discovered at setup, confirmed by the user) plus any user-supplied seeds. Classifies each item, flags items relevant to the user's team using a light WorkIQ-derived profile, and renders a self-contained HTML dashboard. A tightly-bounded fallback web search is used only when a locked source is silent in the window. Domain-agnostic (tax, privacy, AI/ML, healthcare, finance, ESG, labor, etc.). Call this to obtain the full step-by-step contract (inputs, outputs, verification) and then carry out those steps.",
...
SKILL_PROCEDURE = """# Regulation Monitor
...
SKILL_REF = "@cat-agent-skills/regulation_monitor"
SKILL_LICENSE = "MIT"
SKILL_SOURCE = "https://microsoft.github.io/cat-agent-skills/#regulation-monitor"
