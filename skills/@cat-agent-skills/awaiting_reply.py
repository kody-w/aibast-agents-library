"""
Awaiting Reply — RAPP agent, mirrored from awaiting_reply.md
...
Do not edit this file. Edit awaiting_reply.md and re-run the exporter — the gate
re-derives this file and fails if the two have drifted apart.

Redistributed from CAT Agent Skills under MIT
with attribution. Original author: Allan De Castro.
Origin: https://microsoft.github.io/cat-agent-skills/#awaiting-reply
"""
...
__manifest__ = {
    ...
    "description": "Returns the verified RAPP procedure for: A weekday-morning Scout automation that finds emails you sent that asked for something and never got an answer, reports how many are waiting, and prepares follow-up drafts in each thread's own language that it never sends. Call this to obtain the full step-by-step contract (inputs, outputs, verification) and then carry out those steps.",
...
SKILL_PROCEDURE = """# Awaiting Reply
...
SKILL_REF = "@cat-agent-skills/awaiting_reply"
SKILL_LICENSE = "MIT"
SKILL_SOURCE = "https://microsoft.github.io/cat-agent-skills/#awaiting-reply"


class AwaitingReplyAgent(BasicAgent):
